...
Merge Sort | Merge Sort divides the unsorted list into n sublists, each containing one element (a list of one element is considered sorted). It repeatedly merge sublists to produce new sorted sublists until there is only one sublist remaining to get the final sorted list - [Wikipedia](https://en.wikipedia.org/wiki/Merge_sort)
